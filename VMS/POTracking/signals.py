...
def cal_average_response_time(sender, **kwargs):
    ...
    avg_response_time = sum(time_differences) / \
        len(time_differences)  # avg in secs
    # print(avg_response_time)

    # extra formatting if required
    avg_response_time_hrs = (sum(time_differences)/3600) / \
        len(time_differences) if time_differences else 0  # avg in hrs
    ...
    serializer = VendorMetricSerializer(
        instance=vendor_instance, data=data, partial=True)
