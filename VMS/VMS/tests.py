...
class MainTestCase(APITestCase):
    ...
    def test_register(self):
        ...
        response = self.client.post(
            new_url, {"username": "testuser2", "password": "testpass2"}, format='json')
        self.assertEqual(response.status_code, status.HTTP_202_ACCEPTED)

    def test_invalid_register(self):
        new_url = "/api/register"
        response = self.client.post(
            new_url, {"username": "", "password": "testpass"}, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_login(self):
        new_url = "/api/login"
        response = self.client.post(
            new_url, {"username": "testuser", "password": "testpassword"}, format='json')
        self.assertEqual(response.status_code, status.HTTP_202_ACCEPTED)

    def test_invalid_login(self):
        new_url = "/api/login"
        response = self.client.post(
            new_url, {"username": "", "password": "testpassword"}, format='json')
    ...
    def test_add_vendor(self):
        response = self.client.post(self.vendor_url, self.vendor_data,
                                    HTTP_AUTHORIZATION=f'Token {self.token.key}', format='json')
    ...
    def test_empty_name_request(self):
        ...
        err_data["name"] = ""
        response = self.client.post(
            self.vendor_url, err_data, HTTP_AUTHORIZATION=f'Token {self.token.key}', format='json')
    ...
    def test_update_vendor(self):
        ...
        response = self.client.put(
            new_url, new_data, HTTP_AUTHORIZATION=f'Token {self.token.key}')
    ...
    def test_delete_vendor(self):
        ...
        response = self.client.delete(
            new_url, HTTP_AUTHORIZATION=f'Token {self.token.key}')
    ...
    def test_add_po(self):
        ...
        response = self.client.post(
            self.po_url, self.po_data, HTTP_AUTHORIZATION=f'Token {self.token.key}', format='json')

        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(PurchaseOrder.objects.count(), 2)
        self.assertEqual(PurchaseOrder.objects.get(id=2).delivery_date,
                         po_cls_instance.convert_to_timezone(self.po_data["delivery_date"]))
    ...
    def test_add_empty_delivery_date(self):
        ...
        # response=self.client.post(self.vendor_url,self.vendor_data)
        response = self.client.post(
            self.po_url, err_data, HTTP_AUTHORIZATION=f'Token {self.token.key}', format='json')
    ...
    def test_add_empty_items_data(self):
        ...
        # response=self.client.post(self.vendor_url,self.vendor_data)
        response = self.client.post(
            self.po_url, new_data, HTTP_AUTHORIZATION=f'Token {self.token.key}', format='json')
    ...
    def test_update_items(self):
        ...
        response = self.client.put(
            new_url, new_data, HTTP_AUTHORIZATION=f'Token {self.token.key}', format='json')
    ...
    def test_po_ack(self):
        ...
        response = self.client.post(
            new_url, {}, HTTP_AUTHORIZATION=f'Token {self.token.key}', format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertIsNotNone(
            PurchaseOrder.objects.get(id=1).acknowledgment_date)
    ...
    def test_po_quality_rating_avg(self):
        ...
        self.assertEqual(VendorProfile.objects.get(id=2).name, "Intel")
        self.assertEqual(VendorProfile.objects.get(
            id=2).quality_rating_avg, 80.0)
    ...
    def test_po_on_delivery_rate(self):
        ...
        target_datetime = datetime.strptime(
            self.po_data["delivery_date"], "%d-%m-%Y %H:%M")
        if target_datetime > datetime.now():
            self.assertEqual(VendorProfile.objects.get(
                id=2).on_time_delivery_rate, 100)
        else:
            self.assertEqual(VendorProfile.objects.get(
                id=2).on_time_delivery_rate, 0)
